...
export default function DrinkPage() {
  ...
  const drinkName = () => {
    if (drink === false) {
      return (
        <div>
          "Getting the drink"<Spinner animation="border"></Spinner>
        </div>
      );
    ...
    <div className="drinkpage">
      ...
      <Tab.Container id="left-tabs-example" defaultActiveKey="first">
        <Row>
          ...
          <Col sm={4}>
            <Tab.Content>
              <Tab.Pane eventKey="first">{drinkInstr()}</Tab.Pane>
              ...
              <Tab.Pane eventKey="third">
                <p>
                  <Link to={linkCategory}>
                    <Button variant="outline-success">{categoryName}</Button>
                  </Link>
                </p>
                <p>{drinkAlco()}</p>
              </Tab.Pane>
